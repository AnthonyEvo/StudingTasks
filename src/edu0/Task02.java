...
public class Task02 {
	...
	/*
	 * Задачи на линейную и рекурсивну сортировку Комментари в коде, чтобы запустить
	 * доп коменты в линейной сортировке разблокируй закоменченные System.out-ы для
	 * того чтоы сменить размер массива поменяй значение в инциализаторе массива
	 * сразу после этого комента. Если хочешь чтобы рандом был большего диапазона
	 * найди комент про рандом и после % в коде вбей нужное значение, по умолчанию
	 * 10.
	 */
	...
	Task02() { // конструктор класса второй задачи
		...
		System.out.println("Время выполнения линейной сортировки: " + sort01() + "мс\n"); // методы Sort01 и 02
																							// возвращают строку со
																							// временем исполнения

		System.out.println("Время выполнения сортировки разбиением: " + sort02() + "мс"); // с помощью print просто
																							// выводится значение
	}
	...
	void fillArray() { // метод по заполнению массива рандомными числами
		for (int i = 0; i < A.length; i++) {
			...
			A[i] = Math.abs(new Random().nextInt() % 100); // тут задаем рандом, можно сделать диапазон больше или
															// меньше
...
//					System.out.println("  Set B[" + i + "] to A[" + i + "]");
					...
					for (int j = i; j >= 0; j--) { // цикл в котором нужно определить на сколько нужно отступить назад
													// чтобы вставить новое значение

//						System.out.println("  Compare B[" + (j - 1) + "] and A[" + i + "]");

						if (j == 0)
							flag = true; // нужно в случае если цикл дойдет до начала массива и сравнить будет не с чем
						else if (B[j - 1] < A[i])
							flag = false; // сработает если следующее значение тоже меньше записываемого и нужно
											// проверять дальше
						else
							flag = true; // а тут сработает во всех остальных случаях
						...
						if (flag) { // тут происходит запись в отсортированныйй массив и перенос его фрагменто
							...
							for (int k = 0; k < (i - j); k++) { // переносим в буфер значения которые нужно будет
																// сдвинуть
...
//								System.out.println("  Relising to B[" + (j + k + 1) + "] from temp[" + k + "]" );
							...
							temp = null;
							break; // нужен чтобы цикл проверки не крутил еще много раз после выполнения нужных
									// операцияй
		...
		return (System.currentTimeMillis() - startTime) + ""; // тут подсчитываем сколько времени потребовалось чтобы
																// все отсортировать
	}
	...
	String sort02() { // сортировка разбением "самое интересное"
		...
		int B[] = fastSort(A); // запускаем рекурсивный метод fastSort передав ему сгенерированный рандомом
								// массив
	...
	int[] fastSort(int A[]) { // рекурсивный сортировщик с разбиением
		...
		System.out
				.println("  Mid Value is: " + midValue + ", max Value is: " + maxValue + ", min Value is: " + minValue);
		...
		if (A.length > 2) { // случей если массив больше 2
			...
			if (a.length == A.length) { // эти два "если" нужны в случае если все значения в сортируемом массиве равны
										// среднему значению
				return a;
			}
			if (b.length == A.length) { // или он уйдет в бесконечный цикл потому, как все время будет пытаться их
										// разделить
				return b;
			}
			...
			for (int i = 0; i < A.length; i++) { // тут распеределяем полученные результаты
				if (i < a.length) {
					...
					B[i] = bBuf[i - aBuf.length]; // а в "b" наименьшие, возвращаем из в нужном порядке, т.е. от
													// большего к малому
				}
			}
		...
		else if (A.length == 2) { // В случае с двумя значениями все просто, сравним их и поставим сначала большее
									// потом меньшее
