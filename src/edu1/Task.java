...
class Display extends JPanel{
	...
	Display(dType DType) {
		...
		case InputT2Frame:
			...
			label = new JLabel("<html>Задание №2<br> \"Численное решение <br>нелинейных уравнений\"<br> Используется метод<br> деления отрезков<br> пополам.<br> Рабочая формула:<br>\n X^3 + 6X - 1 = 0");
		...
		case InputT3Frame:
			...
			label = new JLabel("<html>Задание №3<br> \"Решение СЛАУ<br>методом Гауса\"<br> Изначальный массив<br> и операции над ним<br> отображаются после запуска\n" );
			label.setPreferredSize(new Dimension(180, 180));
			this.add(label);
			break;
	...
	public void dataInput(double[][] input, dType DType) {
		...
		for(int i = 0; i < input.length; i++) {
			if (maxXValue < input[i][0]) maxXValue = input[i][0];
			if (minXValue > input[i][0]) minXValue = input[i][0];
		}
	}
		...
		if(TType == dType.InputT2Frame || TType == dType.InputT1Frame) {
			...
			for(double i = minXValue - 1; i < maxXValue + 1; i += step) {
				if(TType == dType.InputT1Frame) {
					G.drawLine((int)((i * scale) + sShift),(int) (sShift - (scale * new EqIMN(input, i).getY())), (int)(((i + step) * scale) + sShift), (int)(sShift - (scale * new EqIMN(input, i + step).getY())));
				}
				
				if(TType == dType.InputT2Frame) {
					G.drawLine((int)((i * scale) + sShift), (int)(sShift - (scale * new EqCube(i).getY())), (int)(((i + step) * scale) + sShift), (int)(sShift - (scale * new EqCube(i + step).getY())));
				}
			}
		}
	}
	...
	private void drawPoints(Graphics G) {
		if(TType == dType.InputT2Frame || TType == dType.InputT1Frame) {
			for(int i = 0; i < input.length; i++) {
				...
				G.drawOval((int) ((input[i][0] * scale - 1) + sShift), (int) (sShift - (input[i][1]  * scale + 1)), 3, 3);
				G.drawOval((int) ((input[i][0] * scale - 3) + sShift), (int) (sShift - (input[i][1] * scale + 3)), 7, 7);
			}
		}
	}
...
class EqIMN {
	...
	public EqIMN(double[][] input, double x) {
		Y = input[0][1] + devSubstr(new double[] {input[0][0], input[1][0]}, new double[] {input[0][1], input[1][1]}) * (x - input[0][0]) +
		devSubstr(new double[] {input[0][0], input[1][0], input[2][0]}, new double[] {input[0][1], input[1][1], input[2][1]}) * (x - input[0][0]) * (x - input[1][0]);
	}
...
class EqIML {
	...
	EqIML(double[][] input, double x) {
        ...
        double x1 = input[0][0], x2 = input[1][0], x3 = input[2][0], y1 = input[0][1], y2 = input[1][1], y3 = input[2][1];
